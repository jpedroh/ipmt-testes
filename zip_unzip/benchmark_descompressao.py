...
def main():
  ...
  with open(f'out/benchmark_descompressao_{category}.csv', 'w', newline='') as out_file:
...
def do_benchmark_for_file(input_file_name):
  ...
  ipmt_command = f"/usr/bin/time -f \"%E\" ./bin/ipmt unzip {input_file_name}.myz"
  ...
  zip_command = f"/usr/bin/time -f \"%E\" unzip {input_file_name}.zip -d unzip"
